# This function takes the return value of fastcluster::hclust(), under which height to cluster trees, and above which height to ignore
# This function returns a list, with first element being num_vertices(integer), second element points_in_vertex(a list of vectors).
cutree_manual <- function(tree, hc = NULL, hf = NULL){
  ...
  if (is.null(hc) || is.null(hf))
    stop("under which height to cluster and above which height to ignore must be specified!")
  if (min(height) > hc)
  ...
  for (i in step_hc:1){
    if (row_checkbox[i, 2]){
      ...
      new_vertex <- create_vertex(merge_mat[1:i, ], i)
